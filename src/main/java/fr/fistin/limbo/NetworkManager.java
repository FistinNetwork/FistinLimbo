...
import fr.fistin.limbo.packets.in.PacketIn;
import fr.fistin.limbo.packets.out.PacketOut;
import fr.fistin.limbo.packets.protocols.*;
...
public class NetworkManager
{
    ...
    private void readPacket(SocketChannel socket)
    {
        ...
        {
            System.err.println("Received data on channel not registered. (" + socket.socket().getInetAddress().getHostAddress() + ").");
            return ;
        }
        try
        {
    ...
    public void sendPacket(PlayerConnection playerConnection, PacketOut packetOut)
    {
        try
        {
            ...
            if (id == -1)
            {
                Limbo.getLogger().warning("Sending unknown packet " + packetOut.getClass().getSimpleName());
                return ;
            }
    ...
    public void disconnect(PlayerConnection playerConnection, String message)
    {
        if (playerConnection.getStatus() != Status.STATUS && playerConnection.getStatus() != Status.HANDSHAKE) {
            System.out.println(playerConnection.getInetAddress().getHostString() + (playerConnection.getUserName().isEmpty() ? "" : " (" + playerConnection.getUserName() + ")") + " disconnected" + (message == null ? "" : " : " + message));
        }
